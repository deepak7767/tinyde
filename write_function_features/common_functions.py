def create_hash(primary_key, hash_cols):
    if len(primary_key) == 1:
        ...
        pk_hash_dict = {
            "pk_target_hash_reverse": ', '.join([f"coalesce(cast(target.{c} as string), '')" for c in primary_key[::-1]]),
            "pk_source_hash_reverse": ', '.join([f"coalesce(cast(source.{c} as string), '')" for c in primary_key[::-1]])
        }
    pk_hash_dict['pk_target_hash'] = ', '.join([f"coalesce(cast(target.{c} as string), '')" for c in primary_key])
    pk_hash_dict['pk_source_hash'] = ', '.join([f"coalesce(cast(source.{c} as string), '')" for c in primary_key])
    content_hash_dict = {
        "content_source_hash": ', '.join([f"coalesce(cast(source.{c} as string), '')" for c in hash_cols]),
        "content_target_hash": ', '.join([f"coalesce(cast(target.{c} as string), '')" for c in hash_cols])
    }
    return pk_hash_dict, content_hash_dict

def pre_checks(catalog_name, schema_name, table_name, ddl_path):
    ...
    incremental_dict['pk_hash_dict'], incremental_dict['content_hash_dict'] = create_hash(primary_key, hash_cols=get_cols)
